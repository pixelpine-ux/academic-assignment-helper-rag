...
## 7. Two-Layer Plagiarism Detection

### The Problem
- Exact copy-paste is trivial to detect but easy to bypass with minor edits
- Paraphrased submissions look different on the surface but are semantically identical
- Need both layers: one cheap and instant, one deep and semantic

### Layer 1: Hash-Based Exact Detection (Upload Guard)
...
Runs on every upload before any AI calls. One indexed DB lookup — essentially free.

### Layer 2: Vector Similarity Detection (Semantic Check)
...
### Why Two Layers?
| Layer | Catches | Cost | Action |
|-------|---------|------|--------|
| Hash | Byte-for-byte copies | ~0ms, 1 DB query | Hard reject (409) |
| Vector | Paraphrased content | DB compute per chunk | Soft flag in response |

Hash is a hard reject because identical bytes = no ambiguity. Vector is a soft flag because 0.93 similarity is suspicious, not proof — a human should decide.

### What This Shows
- Layered defence design (cheap check first, expensive check second)
- Reuse of existing embeddings infrastructure (no extra AI calls)
- Distinction between hard errors and soft warnings in API design
